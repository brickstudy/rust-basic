...
impl UserRepository for UserDbRepository {
    ...
    fn find_user_by_id(&self, id: &Box<String>) -> Result<Option<User>, String> {
        ...
        let results = pool.prep_exec(query, mysql::params! { "id" => id.as_str() }).expect("쿼리 실패");
    ...
    fn save_user(&self, user: User) -> Result<(), String> {
        ...
        pool.prep_exec(query, mysql::params! { "id" => user.id.as_str(), "password" => user.password.as_str() }).expect("쿼리 실패");
